...
class PairData(Data):
    ...
    def __init__(self, edge_index_s=None, x_s=None, edge_index_t=None, x_t=None,y_s=None,protein=None,name=None,smiles=None):
        super().__init__()
...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root=None, dataset=None,pro=None,smiles_dic=None,
                 ...
                 pre_transform=None,complex_graph=None,ligand_graph=None,ligand_emb=None):
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process( pro,y,complex_graph,ligand_graph,ligand_emb)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self,pro, y,complex_graph,ligand_graph=None,ligand_emb=None):
        data_list = []
        for name in tqdm(complex_graph, desc="Processing"):

            if name not in y:
                print(f"警告：{name} 不在affinity表中，跳过！")
                continue
            ...
            data = PairData(
                ...
                torch.LongTensor([protein]),name,smiles
                            )
