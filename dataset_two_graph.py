...
class PairData(Data):
    ...
    def __init__(self, edge_index_s=None, x_s=None, edge_index_t=None, x_t=None,y_s=None,protein=None,edge_weights=None):
        super().__init__()
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self,pro, y,complex_graph,ligand_graph=None):
        ...
        for name in tqdm(complex_graph, desc="Processing"):
            ...
            label = y[name]
            protein = pro[name]
            c_size, features, edge_index,edge_weight = complex_graph[name]
            c_size1, features1, edge_index1 = ligand_graph[name]
            data = PairData(
                torch.LongTensor(edge_index).transpose(1, 0), torch.Tensor(features),
                torch.LongTensor(edge_index1).transpose(1, 0), torch.Tensor(features1),
                torch.FloatTensor([label]),
                torch.LongTensor([protein]),
